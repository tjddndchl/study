...
import numpy as np
#pip install statsmodels 3차원 그리기 위한 라이브러리
...
for i in  range(epochs):
    ...
    a1_diff = -(1/len(x1_data)) * sum(x1_data * error) # 오차함수를 a로 미분한값
    a2_diff = -(1 / len(x2_data)) * sum(x2_data * error)  # 오차함수를 a로 미분한값
    b_diff = -(1/len(x1_data)) * sum(y_data - y_hat)          # 오차함수를 b로 미분한 값
